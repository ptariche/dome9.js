...
class dome9 {
  ...
  createIplist(name, description, items) {
    if (!name || !description || !items) throw new Error('name, description, & items required in function createIplist');
    return DOME9_CONFIG({
      method: 'POST',
      uri: 'iplists',
      headers: this.authHeader(),
  ...
  updateIplist(ip, description, items) {
    if (!ip || !name || !description || !items) throw new Error('ip, description, & items required in function updateIplist');
    return DOME9_CONFIG({
      method: 'POST',
      uri: 'iplists/' + ip,
      headers: this.authHeader(),
